...
% task1([1,2,1,5,8,1,0], 5, 1, List). % последние эл-ты не выводятся
% task1([1,5,8,1,0], 5, 1, List). % а здесь выводится
...
task2():-
    ...
    isolate_nl(List, NewList), % выделить nl как отдельное слово
...
% выделение символа перевода строка пробелами:
...
find_max_length([], N, MaxN, Ans):-
    (N @> MaxN, Ans is N; % последнее слово может быть максимальным
    Ans is MaxN), !.
...
% выделяет первое слово из строки (в начале могут идти пробелы)
read_word(String, Word, Tail):-
    get_spaces(String, _, StringWithoutSpaces), % убирает пробелы
    get_word(StringWithoutSpaces, Word, Tail),
    Word \== [].

% преобразовать коды символов в слова:
...
%если слово - не перевод строки, то добавить после него пробел:
check_nl([Head|_]):-
    Head \== 10, !, write(' ').
check_nl(_).